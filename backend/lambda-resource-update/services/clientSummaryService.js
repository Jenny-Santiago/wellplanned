// services/clientSummaryService.js
...
async function recalcularResumenCliente(clienteId) {
    ...
    try {
        ...
        // Aqui solo filtra los que son json
        const workloadFiles = workloadsData.filter(item => {
            return item.Key.endsWith('.json');
        });

        logger.info('Archivos JSON encontrados', { 
            clienteId, 
            totalObjetos: workloadsData.length,
            archivosJson: workloadFiles.length 
        });

        if (!workloadFiles || workloadFiles.length === 0) {
        ...
        // Leer todos los workloads (SOLO LOS ARCHIVOS JSON)
        const workloadsPromises = workloadFiles.map(async (item) => {
            try {
                logger.info('Leyendo workload', { key: item.Key });
                const content = await s3Service.getObject(item.Key);
                
                // El content ya viene parseado desde s3Service.getObject
        ...
        validWorkloads.forEach(workload => {
            logger.info('Procesando workload para extraer año/mes', { 
                id: workload.id,
                periodo: workload.periodo,
            ...
            // Extraer año desde periodo (formato YYYY-MM) o campo year/periodo directo
            let year = null;
            if (workload.periodo) {
                // Si periodo es string "YYYY-MM", extraer año
                if (typeof workload.periodo === 'string' && workload.periodo.includes('-')) {
                    year = workload.periodo.split('-')[0];
                    logger.info('Año extraído de periodo string', { periodo: workload.periodo, year });
                } else {
                    // Si periodo es número (año directo)
                    year = workload.periodo.toString();
                    logger.info('Año extraído de periodo número', { periodo: workload.periodo, year });
                }
            } else if (workload.year) {
                year = workload.year.toString();
                logger.info('Año extraído de campo year', { year });
            }
            
            if (year) {
                añosSet.add(year);
            } else {
                logger.warn('No se pudo extraer año del workload', { workload });
            }
        });

        logger.info('Años y meses extraídos', { 
            años: Array.from(añosSet),
...
async function actualizarResumenEnCliente(clienteId, nuevoResumen) {
    try {
        ...
        const cliente = await s3Service.getObject(clienteKey);
